...
import { createFamilyProxyNode, getCyclePointId } from '@/components/cylc/cheese/cheese-nodes'
import Vue from 'vue'
 ...
 * be reactive and used in the node cheese component).
 *
...
/**
 * @typedef {Object} CylcCheese
 * @property {Map<string, Object>} lookup - lookup map used to access objects without iteration
 * @property {CheeseNode} root - cheese root
 */

/**
 * A data structure representing the cheese used by Cylc.
 *
 ...
 * const rootNode = cylcCheeseInstance.root;
 ...
 * reference.
 *
 * It means that you can easily access any element in the cheese, without
 * having to iterate and visit each of its parents.
 *
 * <script language="javascript">
 * const jobID = 'kinow|five|20150129T1200+12|foo|1'
 * const someJob = cycleCheeseInstance.lookup.get(jobID)
 * </script>
 *
 ...
 *
 * When a node is added, it gets added to the hierarchical cheese (i.e. it
...
class CylcCheese {
  /**
   * Create a cheese with an initial root node, representing
   * a workflow in Cylc.
   *
  ...
  addFamilyProxy (familyProxy) {
    ...
    if (!existingFamilyProxy) {
      ...
      // when linking with the parent node in the cheese, not the new GraphQL data
    ...
    // nodes if they don't exist in the cheese yet, so that we can create the correct hierarchy. Later, we
