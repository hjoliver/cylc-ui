...
import { populateBeefFromGraphQLData } from '@/components/cylc/beef/index'
 ...
 * @param {CylcBeef} beef
 */
function applyDeltasUpdated (updated, beef) {
    ...
    if (updated[updatedKey]) {
      updated[updatedKey].forEach(updatedData => {
        try {
          ...
          const beefFunction = UPDATED[updatedKey][1]
...
/**
 * Handle the initial data burst of deltas. Should create a beef given a workflow from the GraphQL
 * data. This beef contains the base structure to which the deltas are applied to.
 *
 * @param {Deltas} deltas - GraphQL deltas
 * @param {CylcBeef} beef - Beef object backed by an array and a Map
 */
function handleInitialDataBurst (deltas, beef) {
  ...
  populateBeefFromGraphQLData(beef, workflow)
  beef.tallyCyclePointStates()
}

/**
 * Handle the deltas. This function receives the new set of deltas, and the
 * beef object. It is assumed
 * the beef has been correctly created. Except for family proxies, it is expected that other elements
 ...
 * @param {CylcBeef} beef - Beef object backed by an array and a Map
 */
function handleDeltas (deltas, beef) {
  if (deltas.pruned) {
    applyDeltasPruned(deltas.pruned, beef)
  }
  if (deltas.added) {
    applyDeltasAdded(deltas.added, beef)
  }
  if (deltas.updated) {
    applyDeltasUpdated(deltas.updated, beef)
  }
...
export function applyDeltas (deltas, beef) {
  if (deltas && beef) {
    ...
    if (beef.isEmpty()) {
      // When the beef is null, we have two possible scenarios:
      //   1. This means that we will receive our initial data burst in deltas.added.workflow
      //      which we can use to create the beef structure.
      //   2. Or this means that after the shutdown (when we delete the beef), we received a delta.
      //      In this case we don't really have any way to fix the beef.
      ...
      try {
        handleInitialDataBurst(deltas, beef)
      } catch (error) {
        // eslint-disable-next-line no-console
      ...
      // the beef was created, and now the next messages should contain
    ...
    throw Error('Workflow beef subscription did not return data.deltas')
  }
}
