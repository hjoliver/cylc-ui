...
<template>
  <div class="cheeseitem" v-show="filtered">
    <div
      :class="getNodeClass()"
      :style="getNodeStyle()"
    >
    ...
    <span v-show="isExpanded">
      ...
      <CheeseItem
        ...
        v-on:cheese-item-created="$listeners['cheese-item-created']"
        v-on:cheese-item-destroyed="$listeners['cheese-item-destroyed']"
        v-on:cheese-item-expanded="$listeners['cheese-item-expanded']"
        v-on:cheese-item-collapsed="$listeners['cheese-item-collapsed']"
        v-on:cheese-item-clicked="$listeners['cheese-item-clicked']"
      >
...
export default {
  ...
  created () {
    this.$emit('cheese-item-created', this)
  },
  beforeDestroy () {
    this.$emit('cheese-item-destroyed', this)
  },
  beforeMount () {
  ...
  methods: {
    ...
    /**
     * Emits an event `cheese-item-expanded` if `expanded` is true, or emits
     * `cheese-item-collapsed` if `expanded` is false.
     * @param {boolean} expanded whether the node is expanded or not
     */
    emitExpandCollapseEvent (expanded) {
      if (expanded) {
        this.$emit('cheese-item-expanded', this)
      } else {
        this.$emit('cheese-item-collapsed', this)
      }
    },
    getTypeStyle () {
      const styles = {}
    ...
    /**
     * Handler for when any node of the cheese was clicked, except jobs.
     * @param {event} e event
     */
    nodeClicked (e) {
      this.$emit('cheese-item-clicked', this)
    },
    getNodeStyle () {
      return {
