...
<template>
  <div class="beefitem" v-show="filtered">
    <div
      :class="getNodeClass()"
      :style="getNodeStyle()"
    >
    ...
    <span v-show="isExpanded">
      ...
      <BeefItem
        ...
        v-on:beef-item-created="$listeners['beef-item-created']"
        v-on:beef-item-destroyed="$listeners['beef-item-destroyed']"
        v-on:beef-item-expanded="$listeners['beef-item-expanded']"
        v-on:beef-item-collapsed="$listeners['beef-item-collapsed']"
        v-on:beef-item-clicked="$listeners['beef-item-clicked']"
      >
...
export default {
  ...
  created () {
    this.$emit('beef-item-created', this)
  },
  beforeDestroy () {
    this.$emit('beef-item-destroyed', this)
  },
  beforeMount () {
  ...
  methods: {
    ...
    /**
     * Emits an event `beef-item-expanded` if `expanded` is true, or emits
     * `beef-item-collapsed` if `expanded` is false.
     * @param {boolean} expanded whether the node is expanded or not
     */
    emitExpandCollapseEvent (expanded) {
      if (expanded) {
        this.$emit('beef-item-expanded', this)
      } else {
        this.$emit('beef-item-collapsed', this)
      }
    },
    getTypeStyle () {
      const styles = {}
    ...
    /**
     * Handler for when any node of the beef was clicked, except jobs.
     * @param {event} e event
     */
    nodeClicked (e) {
      this.$emit('beef-item-clicked', this)
    },
    getNodeStyle () {
      return {
