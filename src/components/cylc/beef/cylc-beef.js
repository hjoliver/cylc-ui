...
import { createFamilyProxyNode, getCyclePointId } from '@/components/cylc/beef/beef-nodes'
import Vue from 'vue'
 ...
 * be reactive and used in the node beef component).
 *
...
/**
 * @typedef {Object} CylcBeef
 * @property {Map<string, Object>} lookup - lookup map used to access objects without iteration
 * @property {BeefNode} root - beef root
 */

/**
 * A data structure representing the beef used by Cylc.
 *
 ...
 * const rootNode = cylcBeefInstance.root;
 ...
 * reference.
 *
 * It means that you can easily access any element in the beef, without
 * having to iterate and visit each of its parents.
 *
 * <script language="javascript">
 * const jobID = 'kinow|five|20150129T1200+12|foo|1'
 * const someJob = cycleBeefInstance.lookup.get(jobID)
 * </script>
 *
 ...
 *
 * When a node is added, it gets added to the hierarchical beef (i.e. it
...
class CylcBeef {
  ...
  /**
   * Create a beef with an initial root node, representing
   * a workflow in Cylc.
   *
  ...
  constructor (workflow, options) {
    const defaults = {
      cyclePointsOrderDesc: CylcBeef.DEFAULT_CYCLE_POINTS_ORDER_DESC
    }
  ...
  addFamilyProxy (familyProxy) {
    ...
    if (!existingFamilyProxy) {
      ...
      // when linking with the parent node in the beef, not the new GraphQL data
    ...
    // nodes if they don't exist in the beef yet, so that we can create the correct hierarchy. Later, we
