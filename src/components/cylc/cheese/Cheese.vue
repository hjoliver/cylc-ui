...
<template>
  ...
  >
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <v-select
              id="c-cheese-filter-task-states"
              :items="taskStates"
              clearable
              dense
              flat
        ...
        >
          <v-tooltip bottom>
            <template v-slot:activator="{ on }">
              <v-btn
                v-on="on"
                @click="expandAll((cheeseitem) => !['task-proxy', 'job', 'job-details'].includes(cheeseitem.node.type))"
                icon
              >
    ...
    <v-row
      no-gutters
      >
      <!-- each workflow is a cheese root -->
      <cheese-item
        ...
        v-on:cheese-item-created="onCheeseItemCreated"
        v-on:cheese-item-destroyed="onCheeseItemDestroyed"
        v-on:cheese-item-expanded="onCheeseItemExpanded"
        v-on:cheese-item-collapsed="onCheeseItemCollapsed"
        v-on:cheese-item-clicked="onCheeseItemClicked"
      >
...
<script>
import CheeseItem from '@/components/cylc/cheese/CheeseItem'
import Vue from 'vue'
...
export default {
  ...
  methods: {
    ...
    filterNode (node) {
      ...
      this.cheeseItemCache[node.id].filtered = filtered
      return filtered
    },
    removeAllFilters () {
      for (const cheeseitem of Object.values(this.cheeseItemCache)) {
        cheeseitem.filtered = true
      }
    },
    expandAll (filter = null) {
      const collection = filter ? [...Object.values(this.cheeseItemCache)].filter(filter) : Object.values(this.cheeseItemCache)
      for (const cheeseItem of collection) {
        cheeseItem.isExpanded = true
        this.expandedCache.add(cheeseItem)
      }
      this.expanded = true
    },
    collapseAll (filter = null) {
      ...
      for (const cheeseItem of collection) {
        cheeseItem.isExpanded = false
        this.expandedCache.delete(cheeseItem)
      }
      if (!filter) {
        this.expanded = false
      }
    },
    onCheeseItemExpanded (cheeseItem) {
      this.expandedCache.add(cheeseItem)
      this.expanded = true
    },
    onCheeseItemCollapsed (cheeseItem) {
      this.expandedCache.delete(cheeseItem)
    },
    onCheeseItemCreated (cheeseItem) {
      Vue.set(this.cheeseItemCache, cheeseItem.$props.node.id, cheeseItem)
      if (cheeseItem.isExpanded) {
        this.expandedCache.add(cheeseItem)
      }
    },
    onCheeseItemDestroyed (cheeseItem) {
      // make sure the item is removed from all caches, otherwise we will have a memory leak
      Vue.delete(this.cheeseItemCache, cheeseItem.$props.node.id)
      this.expandedCache.delete(cheeseItem)
      this.activeCache.delete(cheeseItem)
    },
    onCheeseItemClicked (cheeseItem) {
      if (this.activable) {
        ...
        cheeseItem.active = !cheeseItem.active
        if (cheeseItem.active) {
          this.activeCache.add(cheeseItem)
        }
      }
    }
  }
