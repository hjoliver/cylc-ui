 ...
 * It is wrapped within a Cheese node (i.e. a cheese-node has a node), for
...
/**
 * A node of the cheese, with the GraphQL data objects organized
 * hierarchically.
 *
 * @typedef {Object} CheeseNode
 * @property {string} id - node ID
 ...
 * @property {Array<CheeseNode>} children - children node
 */

/**
 * @typedef {CheeseNode} WorkflowNode
 */
...
function createWorkflowNode (workflow) {
  // Does not have the infinite-cheese properties (size, state, etc) because this node is used only to build the
 ...
 *
 * @param node {GraphQLData} a cheese node
...
/**
 * @typedef {CheeseNode} CyclePointNode
 */
...
/**
 * @typedef {CheeseNode} FamilyProxyNode
 */
...
/**
 * @typedef {CheeseNode} TaskProxyNode
 */
 ...
 * @returns {boolean}
 */
function containsCheeseData (workflow) {
