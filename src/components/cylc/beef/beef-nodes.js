 ...
 * It is wrapped within a Beef node (i.e. a beef-node has a node), for
...
/**
 * A node of the beef, with the GraphQL data objects organized
 * hierarchically.
 *
 * @typedef {Object} BeefNode
 * @property {string} id - node ID
 ...
 * @property {Array<BeefNode>} children - children node
 */

/**
 * @typedef {BeefNode} WorkflowNode
 */
...
function createWorkflowNode (workflow) {
  // Does not have the infinite-beef properties (size, state, etc) because this node is used only to build the
 ...
 *
 * @param node {GraphQLData} a beef node
...
/**
 * @typedef {BeefNode} CyclePointNode
 */
...
/**
 * @typedef {BeefNode} FamilyProxyNode
 */
...
/**
 * @typedef {BeefNode} TaskProxyNode
 */
 ...
 * @returns {boolean}
 */
function containsBeefData (workflow) {
